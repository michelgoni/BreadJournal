...
final class AppTests: XCTestCase {
    ...
    func tests_save_data() async throws {
        ...
        let store = TestStore(
            ...
            dependencies.journalListDataManager = .mock(
                initialData: try! JSONEncoder().encode(
                    [entry]
                )
            )
            dependencies.journalListDataManager.save = {
                @Sendable [dependencies] data,
                url in
                savedData.setValue(
                    data
                )
                try await dependencies.journalListDataManager.save(
                    data,
                    url
                )
            }
